      ...
      <div class="bg-white p-3 rounded-lg shadow-md">
        <div class="flex flex-row items-center justify-between w-full mb-6 pb-4 border-b border-gray-200">
          <div class="flex items-center gap-2">
            ...
            <h2 class="text-xl font-semibold text-gray-800">{{ currentBot.title }}</h2>
            <span class="px-2 py-1 text-xs font-medium text-emerald-600 bg-emerald-50 rounded-full">
              {{ currentBot.status }}
            </span>
          </div>
          <NuxtLink
        ...
        <ChatBox :bot-config="currentBot" />
      </div>
    </template>
    <div v-else class="text-center py-12">
      <p class="text-gray-600">未找到指定的机器人</p>
      <NuxtLink to="/ai-chat" class="text-emerald-600 hover:text-emerald-700 mt-4 inline-block">
        返回选择
      </NuxtLink>
    </div>
  </div>
...
if (!currentBot.value) {
  showError({ statusCode: 404, message: '未找到指定的机器人' })
}
</script>