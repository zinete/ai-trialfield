<template>
  <UCard
    class="mb-3 hover:shadow-lg transition-all duration-200 border-l-4"
    :class="[
      todo.completed
        ? 'border-l-green-500'
        : 'border-l-blue-500'
    ...
    <div class="flex items-center justify-between">
      ...
      <div class="flex items-center space-x-3 flex-1">
        ...
        <div class="flex-1">
          ...
          <!-- 标签显示 -->
          <div v-if="todoTags" class="flex items-center gap-2 mt-1">
            <UBadge 
              :color="todoTags.color || 'primary'" 
              variant="soft"
              size="xs"
            >
              <UIcon 
...
interface Props {
  todo: SerializedTodos;
  showDetails?: boolean;
}

interface Emits {
  (e: 'toggle-complete', todo: SerializedTodos): void;
  (e: 'delete', todo: SerializedTodos): void;
  (e: 'edit', todo: SerializedTodos): void;
}
...
const todoTags = computed(() => {
  if (!props.todo.tags) return null;
  try {
